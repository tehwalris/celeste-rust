...
## Phase 1: DONE (2026-08-25)

Seven commits on `census` after 70d877f ("delete the walk path (N/7)").
The one decision in Phase 1 was taken as "accept flat_forks": the loop
emitter is gone and the traced kernels ARE the flat-fork kernels - the
checked-in set was already flat, and `traced_kernels_are_current` passed
against it UNCHANGED after the deletion, so no emitted byte moved.

Removed, hand-written (`git diff --numstat 70d877f..HEAD`, minus the
generated files): **-6,150 / +218 lines** over 17 files -
`transpile::kernel` 2,402 -> 146 (the walk evaluator),
`transpile::lower` -589 (the loop-nest emitter), `transpile::fuse` -993,
`transpile::names` -489, `compiled::dispatch` 1,241 -> 170 (the class
registry, fused runner, miss dumps), native-probe -216
(`--kernel-bench`), `bin/transpile` -125, plus the `fused` feature, the
CELESTE_KERNEL_CLASSES / CELESTE_KERNEL_MISS{,_DUMP} / CELESTE_FUSED{,
_UNCOV_DUMP} / CELESTE_TRACED_KERNELS knobs, and the tests
`generated_is_current{,_r20}` and
`compiled_forward_reproduces_the_interpreter` (identical to
`traced_kernels_reproduce_the_interpreter` once there was one kernel
set).

Removed, generated / data: **14,429** lines of `kernel_gen_*.rs` (8
files), **15,095** lines of shape-witness JSON (6 of the 8 files - the
steady and r20-steady witnesses STAY as fixtures for `trace::bind`'s
tests, which rebuild a real boundary shape from them), **13,216**
lines of `rewrites-trace{10,20}*.jsonl` (12 files) and 8 frozen
`*.program.zst`. `crates/celeste-names/src/gen.rs` stays, byte-for-byte
below its header, now marked FROZEN (append-only).

Still referencing deleted things, deliberately left: four `tas/*.txt`
headers that record a `rewrite --recipe rewrites-trace10-dying-*.jsonl
membercheck` gate (provenance of those TAS files, not code); comments in
`rewrites-dying-*.jsonl` naming the trace10 pattern they copied;
historical text in plans/*.md and BENCHMARK_DATA.md.

Side effects to know about: `traced_set_fingerprint()` is a plain `u64`
now (it was `Option`, `None` with the knob off) and the fused
fingerprint is no longer hashed, so the campaign fingerprint of a
CELESTE_COMPILED_FORWARD=1 checkpoint differs from before - a different
engine by construction. Interpreter-only campaigns are unaffected.

Gates: `cargo check --profile quick --all-targets` warning-free at every
commit; the quick-profile suite green (278 passed, 10 skipped);
`traced_kernels_are_current` against the unchanged set.

**Gate 3 FAILED, and the failure predates Phase 1.** The 30-frame
`CELESTE_COMPILED_FORWARD=check bench --frames 30 --deopt` on room (1,0)
under the ladder env stops at frame 25 - the first frame where the lane
count fans out (24 -> 180) - with "38 rows the interpreter produced are
missing from the compiled output (interpreter 94 rows, compiled 56)" on
a 3-lane chunk. With `=1` instead of `check` the run completes at
`kernel lanes: traced 4663 missed 0 plain-routed 0` but the lane counts
diverge from frame 25 (104 vs 180; 3,327 vs 11,774 at f30): the traced
kernels CLAIM the chunk and LOSE rows. The base commit 70d877f, built in
a scratch copy and run with the identical command and
`CELESTE_TRACED_KERNELS=1`, fails at the same frame with the same 38 row
hashes; classes-only (knob unset) passes all 30 frames at `steady 4276
dash 4476 frozen 6498 ... missed 24 plain-routed 24`.

Why the evidence did not catch it: the checked-in traced set was
REGENERATED by 6a2672c (flat forks; kernel0/kernel1 rewritten, 22,838
insertions) at 22:58 on 2026-08-24, after the f94 measurement
BENCHMARK_DATA.md reports for "the traced kernels" - that measurement
is of the loop-fork set - and `traced_kernels_reproduce_the_interpreter`
runs 12 frames, which never leaves the 1-lane prefix.
`traced_kernels_are_current` only says the files match the emitter.

PINNED: the same base code (70d877f) rebuilt with the PRE-flat traced
set from 67f0921 (11,186 / 11,915 / 1,508 lines) passes the identical
30-frame check run - `kernel lanes: ... traced 15274 missed 0
plain-routed 0`, 180 lanes at f25 and 11,774 at f30, equal to the
interpreter. The regression is 6a2672c's flat-fork emission (or its
regeneration), in `transpile::lower::emit_body` - the one function
Phase 1 kept unchanged by design.

Consequence: with the class kernels gone, the compiled engine has NO
passing kernel set past frame 24 until the flat-fork emission is fixed
(or the set is regenerated by a fixed emitter). The interpreter path -
`CELESTE_COMPILED_FORWARD` unset, every ladder campaign today - is
unaffected. The fix belongs to the flat emitter (transpile::lower), not
to Phase 1, and the 12-frame test should become a 30-frame one so the
gate lives in the suite.
