...
| **D1** | ~~One key function~~ **GATED as an equivalence** (2026-08-19): the two constructions are different hashes and never numerically equal, but they induce the SAME lane partition on real data - `native-probe --key-gate` (all 55,958,742 distinct rows of H=68, sidecar-grounded, zero engine-key collisions) and `--key-gate-outputs` (dup-heavy pre-subtract frames, both lineages) both report BIJECTION HOLDS. That certifies `run_frame_chunk`'s internal dedups (engine keys, which drop lanes before the campaign re-keys) lose nothing. "Stops keying 3x" repriced to ~0 by D2's 7 ns/row hash - numeric unification not pursued; see BENCHMARK_DATA.md "D1 GATED" | prerequisite for a shared dedup; if it holds, a compiled run stops keying 3x |
...
| **K2** | Fallback census DONE (2026-08-19): at H=68 room (1,0), ALL 2,935 deopting chunks (399,516 lanes) fail ONE premise - `assert_true %3771` in `anonymous_61` `in_h061_in_k030_and_or_continue_2`, the death/restart path (will_restart=true, empty object array). **Fix (1) - will_restart into pm1 - built, verified and REFUTED by the 2x2 measurement** (see BENCHMARK_DATA.md "K2's pm1 death-partition fix"): dead states already have their own SHAPE and fragments group by shape, so dead lanes never shared a chunk with alive ones - the partition is semantically inert (all four runs' per-frame key sets identical) and costs +2-3% wall. Not landed. The census's real payload stands differently: deopt MACHINERY is ~94% of frame-body CPU at depth (tagged runs 307 s + plain re-runs 241 s of 581 s, for 0.8% of lanes), so the fixes that remain are **K1 (defer/batch the re-runs)** and a **death-shape variant program** (14 of 15 dead frames are a pure delay_restart countdown - trivial dynamics, no premise to fail; shape-keyed dispatch #84 is the landing slot). Option (2), the shape-keyed deopt memo, remains a cheap stopgap | cut deopt volume at the source, not just defer it |
