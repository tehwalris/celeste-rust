//! Room (1,0)'s generated CONSTANT-LATTICE kernels, and nothing
//! hand-written.
//!
...
//! aggregator over this crate and its room00/room20 siblings.
//!
//! Three variant modules (`traced`/`ladder`/`exact`, see
//! `crates/celeste-kernels/src/lib.rs` for what each means), each
//! holding this room's `room10` submodule: one kernel per heap shape
...
//! (the DEFAULT room - these gates run on every commit).
//!
