...
/// The TRACED kernel set for the start room: one module per heap SHAPE
/// the room reaches, rather than one per player class. Generated by
/// `transpile --room-kernels crates/celeste-kernels/src/traced` (which
/// walks the room itself - no recipe, no witness) and checked in like
/// the class kernels above, with `traced_kernels_are_current` as the
/// staleness gate.
///
/// It sits beside them rather than replacing them because the two index
/// differently and OVERLAP rather than nest: a class kernel is per
/// (room, player class) and a traced kernel is per heap shape, so which
/// runs first is a measurement, not a fact about the code.
pub mod traced;
