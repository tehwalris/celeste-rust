...
pub const SHAPE: u64 = 16620777245043620552;
...
pub fn bind(b: &Rt2) -> Option<(Uni, RowSlots)> {
    ...
    let u = Uni {
        c299: match &b.cols[cell("objects[0].hitbox.h")? as usize] { Col::U(AV::Num(n)) => *n, _ => return None },
        c300: match &b.cols[cell("objects[0].hitbox.w")? as usize] { Col::U(AV::Num(n)) => *n, _ => return None },
        c301: match &b.cols[cell("objects[0].hitbox.x")? as usize] { Col::U(AV::Num(n)) => *n, _ => return None },
        c302: match &b.cols[cell("objects[0].hitbox.y")? as usize] { Col::U(AV::Num(n)) => *n, _ => return None },
        c309: match &b.cols[cell("objects[1].hitbox.h")? as usize] { Col::U(AV::Num(n)) => *n, _ => return None },
        c310: match &b.cols[cell("objects[1].hitbox.w")? as usize] { Col::U(AV::Num(n)) => *n, _ => return None },
        c311: match &b.cols[cell("objects[1].hitbox.x")? as usize] { Col::U(AV::Num(n)) => *n, _ => return None },
        c312: match &b.cols[cell("objects[1].hitbox.y")? as usize] { Col::U(AV::Num(n)) => *n, _ => return None },
    };
    let s = RowSlots {
        c87: cell("deaths")?,
        ...
        c297: cell("objects[0].flip.x")?,
        c298: cell("objects[0].flip.y")?,
        c241: cell("objects[0].hide_for")?,
        c242: cell("objects[0].hide_in")?,
        c303: cell("objects[0].rem.x")?,
        c304: cell("objects[0].rem.y")?,
        c250: cell("objects[0].solids")?,
        c305: cell("objects[0].spd.x")?,
        c306: cell("objects[0].spd.y")?,
        c252: cell("objects[0].spr")?,
        c254: cell("objects[0].x")?,
        c255: cell("objects[0].y")?,
        c258: cell("objects[1].collideable")?,
        c307: cell("objects[1].flip.x")?,
        c308: cell("objects[1].flip.y")?,
        c260: cell("objects[1].hide_for")?,
        c261: cell("objects[1].hide_in")?,
        c313: cell("objects[1].rem.x")?,
        c314: cell("objects[1].rem.y")?,
        c269: cell("objects[1].solids")?,
        c315: cell("objects[1].spd.x")?,
        c316: cell("objects[1].spd.y")?,
...
/// `append` closes the key with `mix64(KPART + kv.h)`, which is
/// byte-identical to `Rt2::boundary`'s own row key.
pub const KPART1_1: u64 = 16512470659984499207;
pub const KPART2_1: u64 = 13069243305972868448;
...
pub fn frame(u: &Uni, rin: &RowsIn, g: &G, sink: &mut dyn Sink) -> u16 {
    ...
    let n67: ZB = zb_not(r_c297);
    let n68: ZB = zn_eq(zn_splat(P8::from_raw(0i32)), r_c242);
    let n69: bool = P8::from_raw(0i32) == u.c301;
    let n70: bool = P8::from_raw(0i32) == u.c302;
    let n71: ZB = zn_eq(zn_splat(P8::from_raw(0i32)), r_c304);
    let n72: ZB = zb_not(r_c307);
    let n73: ZB = zb_not(r_c308);
    let n74: bool = P8::from_raw(0i32) == u.c311;
    let n75: bool = P8::from_raw(0i32) == u.c312;
    let n76: ZB = zn_eq(zn_splat(P8::from_raw(0i32)), r_c314);
    let n77: ZB = zn_eq(zn_splat(P8::from_raw(0i32)), r_c316);
    let n78: ZB = zn_eq(zn_splat(P8::from_raw(6815744i32)), r_c273);
    let n81: ZN = zn_add(zn_splat(P8::from_raw(65536i32)), r_c84);
    let n82: ZN = zn_rem(n81, zn_splat(P8::from_raw(1966080i32)));
    let n83: ZB = zn_eq(zn_splat(P8::from_raw(0i32)), n82);
    let n84: ZB = zn_eq(zn_splat(P8::from_raw(0i32)), r_c260);
    let n85: bool = P8::from_raw(524288i32) == u.c309;
    let n86: ZB = zn_eq(zn_splat(P8::from_raw(0i32)), r_c313);
    let n87: ZB = zn_eq(zn_splat(P8::from_raw(0i32)), r_c315);
    let n88: ZB = zb_not(r_c43);
    let n89: ZB = zn_gt(r_c20, zn_splat(P8::from_raw(0i32)));
    let n90: ZB = zn_le(r_c20, zn_splat(P8::from_raw(0i32)));
    let n91: ZB = zn_gt(r_c39, zn_splat(P8::from_raw(0i32)));
    let n92: ZN = zn_sub(r_c39, zn_splat(P8::from_raw(65536i32)));
    let n93: ZB = zn_le(n92, zn_splat(P8::from_raw(0i32)));
    let n94: ZB = zn_eq(zn_splat(P8::from_raw(0i32)), r_c261);
    let n95: bool = P8::from_raw(524288i32) == u.c310;
    let n96: ZB = zn_eq(zn_splat(P8::from_raw(7340032i32)), r_c274);
    let n97: ZB = zn_eq(zn_splat(P8::from_raw(1179648i32)), r_c271);
    let n98: ZB = zb_not(r_c42);
    let n99: ZB = zn_eq(zn_splat(P8::from_raw(65536i32)), r_c88);
    let n100: ZB = zb_not(r_c298);
    let n101: ZB = zn_eq(zn_splat(P8::from_raw(0i32)), r_c241);
    let n102: bool = P8::from_raw(524288i32) == u.c299;
    let n103: bool = P8::from_raw(524288i32) == u.c300;
    let n104: ZB = zn_eq(zn_splat(P8::from_raw(0i32)), r_c303);
    let n105: ZB = zn_eq(zn_splat(P8::from_raw(0i32)), r_c305);
    let n106: ZB = zn_eq(zn_splat(P8::from_raw(0i32)), r_c306);
    let n107: ZB = zn_eq(zn_splat(P8::from_raw(1179648i32)), r_c252);
    let n108: ZB = zn_eq(zn_splat(P8::from_raw(2621440i32)), r_c254);
    let n109: ZB = zn_eq(zn_splat(P8::from_raw(7340032i32)), r_c255);
    let n110: ZN = zn_add(zn_splat(P8::from_raw(65536i32)), r_c86);
    let n131: ZN = zn_add(zn_splat(P8::from_raw(65536i32)), r_c85);
    let n132: ZN = zn_rem(n131, zn_splat(P8::from_raw(3932160i32)));
    let n133: ZB = zn_eq(zn_splat(P8::from_raw(0i32)), n132);
    let n134: ZN = zsel_n(n133, n110, r_c86);
    let n135: ZN = zsel_n(n83, n134, r_c86);
    let n136: ZN = zsel_n(n83, n132, r_c85);
    let n155: ZB = zb_and(n90, n91);
    let n158: ZN = zn_sub(r_c20, zn_splat(P8::from_raw(65536i32)));
    let n159: ZN = zsel_n(n89, n158, r_c20);
    let n160: ZB = zn_le(r_c39, zn_splat(P8::from_raw(0i32)));
    let n161: ZB = zn_gt(n92, zn_splat(P8::from_raw(0i32)));
    let n162: ZN = zsel_n(n91, n92, r_c39);
    let n163: ZN = zsel_n(n89, r_c39, n162);
    let n164: ZB = zb_and(n90, n160);
    let n165: ZB = zb_and(n155, n161);
    let n166: ZB = zb_or(n164, n165);
    let n167: ZB = zb_or(n89, n166);
    let n169: ZW = zw_cellmix_n(20u64, r_c20, 1542469173u64);
    let n170: ZW = zw_cellmix_n(20u64, r_c20, 668265263u64);
    let n171: ZW = zw_add(zw_splat(0u64), n169);
    let n172: ZW = zw_add(zw_splat(0u64), n170);
    let n173: ZW = zw_cellmix_n(39u64, n92, 1542469173u64);
    let n174: ZW = zw_cellmix_n(39u64, n92, 668265263u64);
    let n175: ZW = zw_add(n171, n173);
    let n176: ZW = zw_add(n172, n174);
    let n177: ZW = zw_cellmix_n(84u64, n82, 1542469173u64);
    let n178: ZW = zw_cellmix_n(84u64, n82, 668265263u64);
    let n179: ZW = zw_add(n175, n177);
    let n180: ZW = zw_add(n176, n178);
    let n181: ZW = zw_cellmix_n(85u64, n136, 1542469173u64);
    let n182: ZW = zw_cellmix_n(85u64, n136, 668265263u64);
    let n183: ZW = zw_add(n179, n181);
    let n184: ZW = zw_add(n180, n182);
    let n185: ZW = zw_cellmix_n(86u64, n135, 1542469173u64);
    let n186: ZW = zw_cellmix_n(86u64, n135, 668265263u64);
    let n187: ZW = zw_add(n183, n185);
    let n188: ZW = zw_add(n184, n186);
    let n189: ZW = zw_cellmix_n(87u64, r_c87, 1542469173u64);
    let n190: ZW = zw_cellmix_n(87u64, r_c87, 668265263u64);
    let n191: ZW = zw_add(n187, n189);
    let n192: ZW = zw_add(n188, n190);
    let n193: ZW = zw_cellmix_n(20u64, n159, 1542469173u64);
    let n194: ZW = zw_cellmix_n(20u64, n159, 668265263u64);
    let n195: ZW = zw_add(zw_splat(0u64), n193);
    let n196: ZW = zw_add(zw_splat(0u64), n194);
    let n197: ZW = zw_cellmix_n(39u64, n163, 1542469173u64);
    let n198: ZW = zw_cellmix_n(39u64, n163, 668265263u64);
    ...
    let n205: ZW = zw_add(n203, n177);
    let n206: ZW = zw_add(n204, n178);
    let n207: ZW = zw_add(n205, n181);
    let n208: ZW = zw_add(n206, n182);
    let n209: ZW = zw_add(n207, n185);
    let n210: ZW = zw_add(n208, n186);
    let n211: ZW = zw_add(n209, n189);
    let n212: ZW = zw_add(n210, n190);
    let ok_v0_b0: u16 = ALL & zb_holds(r_c38) & zb_holds(n88) & zb_holds(n96) & zb_holds(n78) & zb_holds(n97) & zb_holds(n77) & zb_holds(n87) & zb_holds(r_c269) & zb_holds(n76) & zb_holds(n86) & zb_holds(n94) & zb_holds(n84) & zb_holds(n73) & zb_holds(n72) & zb_holds(r_c258) & zb_holds(n109) & zb_holds(n108) & zb_holds(n107) & zb_holds(n106) & zb_holds(n105) & zb_holds(r_c250) & zb_holds(n71) & zb_holds(n104) & zb_holds(n68) & zb_holds(n101) & zb_holds(n100) & zb_holds(n67) & zb_holds(r_c239) & zb_holds(n99) & zb_holds(r_c175) & zb_holds(n98);
    let bd_v0_b0: bool = !n75 || !n74 || !n95 || !n85 || !n70 || !n69 || !n103 || !n102;
    let live_v0_b0: u16 = ALL & zb_holds(n93) & zb_holds(n90) & zb_holds(n91);
    let ok_v0_b1: u16 = ALL & zb_holds(r_c38) & zb_holds(n88) & zb_holds(n96) & zb_holds(n78) & zb_holds(n97) & zb_holds(n77) & zb_holds(n87) & zb_holds(r_c269) & zb_holds(n76) & zb_holds(n86) & zb_holds(n94) & zb_holds(n84) & zb_holds(n73) & zb_holds(n72) & zb_holds(r_c258) & zb_holds(n109) & zb_holds(n108) & zb_holds(n107) & zb_holds(n106) & zb_holds(n105) & zb_holds(r_c250) & zb_holds(n71) & zb_holds(n104) & zb_holds(n68) & zb_holds(n101) & zb_holds(n100) & zb_holds(n67) & zb_holds(r_c239) & zb_holds(n99) & zb_holds(r_c175) & zb_holds(n98);
    let bd_v0_b1: bool = !n75 || !n74 || !n95 || !n85 || !n70 || !n69 || !n103 || !n102;
    let live_v0_b1: u16 = ALL & zb_holds(n167);
    let sh0 = KShared0 {
        c87: r_c87,
        c39: n92,
        c84: n82,
