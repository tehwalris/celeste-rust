    ...
    loop {
        ...
        match terminator {
            ...
            Terminator::ConditionalSkip {
                ...
                // Route whole states: a state whose condition is uniformly
                // false skips the guarded region; anything else enters it.
                // No filtering, no duplication - a mixed mask enters and
                // the region's masked stores handle the false lanes, same
                // as when no skip exists.
                let states = match flow_data {
                    FlowData::States(states) => states,
                    FlowData::StatesAndReturns(_) => {
                        return Err(anyhow::anyhow!(
                            "Return value at a ConditionalSkip"
                        ))
                    }
                };
                ...
                for state in states {
                    let uniformly_false = matches!(
                        state.local_env.get(*condition),
                        crate::interpreter::value::Value::Bool(
                ...
                let source_label = block_label
                    .clone()
                    .unwrap_or_else(|| Label::from("__entry".to_string()));
                for (target, edge_states) in
                    [(enter_target, enter_states), (skip_target, skip_states)]
                {
                    ...
                    let edge_flow = FlowData::States(edge_states);
                    let bound_phi = adapter.flow_block_phi(&source_label, target_block)?;
                    let edge_flow = bound_phi.flow(edge_flow)?;
                    let bound_before_join =
                        adapter.flow_block_before_join(&fake_liveness, target_block)?;
                    let edge_flow = bound_before_join.flow(edge_flow)?;
                    queue_for_block(
                        target,
                        edge_flow,
                        Some(block_dag_id),
...
mod conditional_skip_tests {
    ...
    /// entry: load mask -> %0, input -> %1; skip_if_none %0 ? end : region
    /// region: %2 = %1 + %1; br end
    /// end: %3 = phi [entry: %1, region: %2]; return %3
    fn skip_cfg() -> Cfg {
        let id = LocalId::from;
        let entry = Block {
            instructions: vec![
            ...
            terminator: (
                ...
                Terminator::ConditionalSkip {
                    condition: id(0),
                    skip_target: Label::from("end".to_string()),
                    enter_target: Label::from("region".to_string()),
                },
            ),
            ...
            instructions: vec![(
                ...
                Instruction::Phi {
                    branches: vec![
                        ...
                        (Label::from("region".to_string()), id(2)),
                    ],
                },
            )],
            terminator: (id(12), Terminator::Return { value: Some(id(3)) }),
            hint_normalize: false,
        };
        let mut named = std::collections::HashMap::default();
        named.insert(Label::from("region".to_string()), region);
        named.insert(Label::from("end".to_string()), end);
        Cfg {
            entry,
            named,
            slots: std::sync::Arc::new(crate::ir::SlotMap::identity()),
        }
    }

    fn run(mask: Value) -> Value {
        use crate::interpreter::value::HeapValue;
        let fixed_env = FixedEnv::new();
        let mut state = State::new();
        state.vector_size = 2;
        let mask_cell = state.heap.alloc();
        state.heap.set(mask_cell, HeapValue::Value(mask));
        state.global_env.insert("mask".to_string(), mask_cell);
        let input_cell = state.heap.alloc();
        state.heap.set(
            input_cell,
            HeapValue::Value(Value::Number(MaybeVector::Vector(std::sync::Arc::new(
                vec![Pico8Num::from_i16(3), Pico8Num::from_i16(4)],
            )))),
        );
        state.global_env.insert("input".to_string(), input_cell);
        let results = interpret_cfg(skip_cfg(), state, &fixed_env).unwrap();
        assert_eq!(results.len(), 1, "one state, never split");
        results[0].1.clone().unwrap()
    }

    /// Uniformly false mask: the region is skipped, the phi takes the
    /// entry value.
    #[test]
    fn uniformly_false_skips() {
        let out = run(Value::Bool(MaybeVector::Scalar(false)));
        match out {
            Value::Number(MaybeVector::Vector(v)) => {
                assert_eq!(&v[..], &[Pico8Num::from_i16(3), Pico8Num::from_i16(4)]);
            }
            other => panic!("expected untouched vector, got {:?}", other),
        }
    }

    /// A mixed mask enters the region with the whole state - no filtering,
    /// no duplication.
    #[test]
    fn mixed_enters_whole() {
        let out = run(Value::Bool(MaybeVector::Vector(std::sync::Arc::new(vec![
            true, false,
        ]))));
        match out {
            Value::Number(MaybeVector::Vector(v)) => {
                assert_eq!(&v[..], &[Pico8Num::from_i16(6), Pico8Num::from_i16(8)]);
            }
            other => panic!("expected doubled vector, got {:?}", other),
        }
    }

    /// A uniformly true mask also enters (skip only fires on uniform
    /// false).
    #[test]
    fn uniformly_true_enters() {
        let out = run(Value::Bool(MaybeVector::Scalar(true)));
        match out {
            Value::Number(MaybeVector::Vector(v)) => {
                assert_eq!(&v[..], &[Pico8Num::from_i16(6), Pico8Num::from_i16(8)]);
            }
