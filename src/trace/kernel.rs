...
fn write_lattice_set(
    ...
    let room = kernel_room_mod();
    let sizes = write_kernels_from_refs(&refs, &dir.join(&room))?;
    write_room_variant_wrapper(dir, &room)?;
    Ok(sizes)
}

/// Write the THIN wrapper that sits above one room's kernels inside a
/// per-room kernel crate (`crates/celeste-kernels-room<xy>/src/<variant>/
/// mod.rs`): just `pub mod room<xy>;`. It used to be folded into the
/// top-level `mod.rs` that also carried `SETS`/`FINGERPRINT`
/// (`merge_kernel_sets`) - that part moved up to the aggregator crate
/// (`crates/celeste-kernels`) when each room became its own crate, so
/// this is what is left on the room-crate side, generated for the same
/// reason: nothing here should be hand-maintained separately from the
/// room directory it names.
fn write_room_variant_wrapper(dir: &std::path::Path, room: &str) -> Result<()> {
    std::fs::create_dir_all(dir)?;
...
/// Every per-room kernel crate under `root/crates`, sorted by room name:
/// `crates/celeste-kernels-room00` -> `("room00", that path)`. The merge
/// step's staleness gate discovers rooms this way, so a new room crate
/// is picked up without a code change here (`regen-generated.sh` still
/// needs its `ROOMS` list extended to GENERATE one).
pub fn room_kernel_crate_dirs(
    root: &std::path::Path,
) -> Result<Vec<(String, std::path::PathBuf)>> {
    let crates_dir = root.join("crates");
    let mut out: Vec<(String, std::path::PathBuf)> = std::fs::read_dir(&crates_dir)
        ...
        .filter_map(|e| {
            ...
            let suffix = name.strip_prefix("celeste-kernels-room")?;
            Some((format!("room{}", suffix), e.path()))
        })
        .collect();
    out.sort();
    Ok(out)
...
/// Write the AGGREGATOR `mod.rs` of a MULTI-ROOM kernel set
/// (`crates/celeste-kernels/src/<set>/mod.rs`): one `pub use` per
/// generated room's crate plus `SETS`, the list of every room's
/// `KERNELS` table, which `Dispatch::new_multi` flattens into the one
/// shape-hash registry (a cross-room hash collision is refused there,
/// at startup). The rooms are generated one process each
/// (`write_room_kernels*`) into separate room crates, so the merge is a
/// separate, file-level step that only reads what they wrote.
///
/// `FINGERPRINT` re-hashes every room's kernel sources in room-name
/// order, so it changes whenever any room's set does and two campaigns
/// with different sets can never share a checkpoint. It depends only on
/// the `kernelN.rs` file CONTENTS, not their path, so relocating a
/// room's kernels into its own crate does not change this value.
pub fn merge_kernel_sets(
    set: &str,
    out_dir: &std::path::Path,
    room_dirs: &[std::path::PathBuf],
) -> Result<Vec<String>> {
    let (rooms, m) = merged_mod_rs(set, room_dirs)?;
    std::fs::create_dir_all(out_dir)?;
    std::fs::write(out_dir.join("mod.rs"), m)?;
    Ok(rooms)
}

/// What `merge_kernel_sets` would write, without writing it. `set` is
/// the variant name (`traced`/`ladder`/`exact`) - it names the
/// submodule each room crate exposes it under
/// (`celeste_kernels_room00::traced::room00`). Each entry of
/// `room_dirs` is a directory containing exactly one `room<x><y>/`
/// subdirectory (a room crate's `src/<set>`, or a matching scratch
/// directory `write_room_kernels*` just wrote). The staleness gates
/// pass the COMMITTED room crates' directories, which catches ANY
/// room's kernel sources drifting from the recorded fingerprint -
/// including the rooms the fast gates do not regenerate.
pub fn merged_mod_rs(
    set: &str,
    room_dirs: &[std::path::PathBuf],
) -> Result<(Vec<String>, String)> {
    let mut entries: Vec<(String, std::path::PathBuf)> = Vec::new();
    for rd in room_dirs {
        let found = std::fs::read_dir(rd)
            .with_context(|| format!("read {}", rd.display()))?
            .filter_map(|e| {
                let e = e.ok()?;
                let name = e.file_name().to_string_lossy().into_owned();
                (e.path().is_dir() && name.starts_with("room")).then_some((name, e.path()))
            })
            .next();
        let (room, path) = found
            .ok_or_else(|| anyhow::anyhow!("no room*/ kernel directory under {}", rd.display()))?;
        entries.push((room, path));
    }
    entries.sort();
    anyhow::ensure!(!entries.is_empty(), "no room directories given - generate per-room sets first");
    let rooms: Vec<String> = entries.iter().map(|(r, _)| r.clone()).collect();
    let fingerprint = {
        use std::hash::{Hash, Hasher};
        let mut h = rustc_hash::FxHasher::default();
        for (room, path) in &entries {
            room.hash(&mut h);
            let mut files: Vec<(usize, std::path::PathBuf)> = std::fs::read_dir(path)?
                .filter_map(|e| {
                    let p = e.ok()?.path();
    ...
    for room in &rooms {
        writeln!(m, "pub use celeste_kernels_{room}::{set}::{room};", room = room, set = set)?;
    }
