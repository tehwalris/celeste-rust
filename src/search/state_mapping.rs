...
mod tests {
    ...
    /// say, an init error would leave the mapping untested while looking green.
    fn disk_available() -> bool {
        std::path::Path::new("lua/celeste-minimal.lua").exists()
            && std::path::Path::new("rewrites.jsonl").exists()
    }

    /// A canonical state with real closures in it: the plain program's heap
    /// after init.
    fn plain_init_state() -> State {
        let program = Program::compile_from_disk().expect("compile plain program");
        let fixed_env = program.fixed_env();
        let initial = create_initial_state_with_builtins(&fixed_env);
        let states =
            interpret_cfg(program.init_cfg().clone(), initial, &fixed_env).expect("run init");
        states.into_iter().next().expect("init produced a state").0
    }

    fn mapping_from_disk() -> StateMapping {
        let recipe = Recipe::load("rewrites.jsonl").expect("load recipe");
        let mapping = StateMapping::from_recipe(&recipe);
        assert!(
            !mapping.is_identity(),
            "the checked-in recipe should contain promote_capture entries"
        );
        mapping
    }

    /// Round trip through both directions preserves the observed state, and
    /// each direction actually changes the representation (so the test cannot
    /// silently pass on a no-op mapping).
    #[test]
    fn roundtrip_preserves_observation() {
        if !disk_available() {
            return;
        }
        let (canonical, mapping) = (plain_init_state(), mapping_from_disk());
        let baseline = observe_state(&canonical);

        let mut specialized = canonical.clone();
        mapping.from_canonical(&mut specialized).expect("from_canonical");
        assert_eq!(observe_state(&specialized), baseline, "from_canonical changed meaning");

        let mut back = specialized.clone();
        mapping.to_canonical(&mut back).expect("to_canonical");
        assert_eq!(observe_state(&back), baseline, "to_canonical changed meaning");

        let mut again = back.clone();
        mapping.from_canonical(&mut again).expect("from_canonical after roundtrip");
        assert_eq!(observe_state(&again), baseline);
    }

    /// Each direction refuses a state already in its target representation.
    /// Silent double-conversion is the bug class this guards against.
    #[test]
    fn refuses_wrong_representation() {
        if !disk_available() {
            return;
        }
        let (canonical, mapping) = (plain_init_state(), mapping_from_disk());
        ...
        // Unboxing an already-unboxed state must fail loudly.
        assert!(mapping.from_canonical(&mut specialized.clone()).is_err());
        // Reboxing an already-boxed state must fail loudly.
        assert!(mapping.to_canonical(&mut canonical.clone()).is_err());
    }
}
