...
//! traced kernels (one crate per room,
//! `crates/celeste-kernels-room00/src/traced/` etc., aggregated by
//! `crates/celeste-kernels`). The CLI is `src/bin/transpile.rs`.
//!
...
//! and its room crates, whose contents it produces. So if you change what
//! the emitter emits, the checked-in kernels stop compiling and
//! `cargo build --bin transpile` cannot build the tool that would fix
//! them. The recovery is `git checkout crates/celeste-kernels/src
//! crates/celeste-kernels-room00/src crates/celeste-kernels-room10/src
//! crates/celeste-kernels-room20/src`, build, regenerate; or use
